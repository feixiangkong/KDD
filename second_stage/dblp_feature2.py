...
class Process_data(object):
    def __init__(self, paper_dic, mode, paper_info_more, seed=42):
        ...
        if (
            ...
            # )  # 最少选一个负例，最多选 references 的数量（因为有时正例数量多于 references！）
    ...
    def get_cur_paper_title(self, cur_pid, bs):
        """
    ...
    def get_referenced_place_num(self, paper_id):
        ...
        if title == "":
            try:
                ref_file = join(f"../dataset/PST/paper-xml/{paper_id}.xml")
                ref_f = open(ref_file, encoding="utf-8")
                xml = ref_f.read()
                bs = BeautifulSoup(xml, "xml")
                ref_f.close()
                self.get_cur_paper_title(paper_id, bs)
