...
def str_normalize(s):
    ...
    ## remove_diacritics 发音区分符号和重音符号删除
...
def get_surrounding_numbers(n, m):
    """
    指定的数字前后各取 m 个数字，并返回包含这些数字的列表。
    列表的元素个数为 m，负数不会包含在结果中。

    :param n: 基准正整数
    :param m: 列表的元素个数
    :return: 包含 n 前后 m//2 个数的列表
    """
    if m % 2 == 0:
